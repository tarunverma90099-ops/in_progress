...
import { AttendanceRecord, Class, Session, Student, StudentAttendance, User, resetAll } from './memory-models';
...
// --- Login (regression: "invalid email or password" with correct credentials) ---

/** Write a user row directly, bypassing Mongoose setters, as legacy/bulk data does. */
async function seedRawUser(email: string, plainPassword: string) {
  resetAll();
  const bcrypt = (await import('bcryptjs')).default;
  return User.create({
    email, // stored verbatim — NOT normalised
...
test('login succeeds when the stored email differs in case', async () => {
  await seedRawUser('Nitish@Student.EDU', 'password123');
  const res = await post(h(loginRoute), '/api/auth/login', {
...
test('login succeeds when the stored email has stray whitespace', async () => {
  await seedRawUser('  nitish@student.edu  ', 'password123');
  const res = await post(h(loginRoute), '/api/auth/login', {
    email: 'nitish@student.edu',
    password: 'password123',
...
test('login succeeds when the user types the email in a different case', async () => {
  await seedRawUser('nitish@student.edu', 'password123');
  const res = await post(h(loginRoute), '/api/auth/login', {
    email: '  NiTiSh@Student.edu ',
    password: 'password123',
...
test('login still rejects a genuinely wrong password', async () => {
  await seedRawUser('Nitish@Student.EDU', 'password123');
  const res = await post(h(loginRoute), '/api/auth/login', {
    email: 'nitish@student.edu',
...
test('login does not crash when the stored hash is missing', async () => {
  resetAll();
  await User.create({ email: 'broken@c.edu', name: 'B', role: 'student' }); // no password
  const res = await post(h(loginRoute), '/api/auth/login', {
    email: 'broken@c.edu',
    password: 'password123',
    role: 'student',
  });
  assert.equal(res.status, 401, 'a malformed row is a failed login, not a 500');
});

test('login rejects an email regex injected as a wildcard', async () => {
  await seedRawUser('nitish@student.edu', 'password123');
  const res = await post(h(loginRoute), '/api/auth/login', {
    email: '.*',
    password: 'password123',
  ...
  assert.equal(res.status, 401, 'regex metacharacters must be escaped, not matched');
});

test('registering an email that differs only by case is a conflict', async () => {
  resetAll();
  const base = { password: 'password123', role: 'student', rollNo: '101' };
  const first = await post(h(register), '/api/auth/register', { ...base, name: 'A', email: 'alice@c.edu' });
  assert.equal(first.status, 201);
  const second = await post(h(register), '/api/auth/register', { ...base, name: 'A2', email: 'ALICE@C.EDU' });
  assert.equal(second.status, 409, 'case variants must not create a second account');
});
...
test('marking a full roster scales without a query per student', async () => {
  resetAll();
  const teacher = await User.create({ name: 'T', email: 't@c.edu', role: 'teacher' });
  const cls = await Class.create({ name: 'Big Class', teacherId: teacher._id, capacity: 200 });

  const students = [];
  for (let i = 0; i < 60; i += 1) {
    const u = await User.create({ name: `S${i}`, email: `s${i}@c.edu`, role: 'student' });
    await post(h(enroll), '/api/enrollments', { userId: u._id, classId: cls._id });
    students.push(u);
  }

  // Count reads/writes issued while marking the whole roster in one request.
  const findSpy = { count: 0 };
  const originalFind = Student.find.bind(Student);
  Student.find = ((q: Record<string, unknown>) => {
    findSpy.count += 1;
    return originalFind(q);
  }) as typeof Student.find;
  ...
  assert.ok(
    findSpy.count <= 2,
    `expected a bounded number of Student.find calls, got ${findSpy.count}`
  );
  assert.equal(AttendanceRecord.docs.length, 60);

  const present = AttendanceRecord.docs.filter((r) => r.status === 'Present').length;
  assert.equal(present, 45);

  // Aggregates must still be exact after a bulk write.
  const first = await Student.findOne({ userId: students[0]._id, classId: cls._id });
  assert.equal(first!.attended, 0, 'index 0 was marked Absent');
  assert.equal(first!.total, 1);
  const second = await Student.findOne({ userId: students[1]._id, classId: cls._id });
  assert.equal(second!.attended, 1);
  assert.equal(second!.total, 1);

  const cls2 = await Class.findById(cls._id);
  assert.equal((cls2!.sessionHistory as { present: number }[])[0].present, 45);
});

test('bulk re-marking corrects aggregates without inflating totals', async () => {
  const { cls, alice, bob } = await seed();
  await post(h(enroll), '/api/enrollments', { userId: alice._id, classId: cls._id });
  await post(h(enroll), '/api/enrollments', { userId: bob._id, classId: cls._id });

  const mark = (aliceStatus: string, bobStatus: string) =>
    post(h(markAttendance), '/api/attendance', {
      classId: cls._id,
      date: '2026-03-02',
      attendanceData: [
        { studentId: alice._id, status: aliceStatus },
        { studentId: bob._id, status: bobStatus },
      ],
    });

  await mark('Present', 'Present');
  await mark('Absent', 'Present'); // correct Alice only
  ...
  assert.equal(aliceRow!.attended, 0);
  assert.equal(aliceRow!.total, 1, 'total must not grow on a correction');
  assert.equal(bobRow!.attended, 1);
  assert.equal(bobRow!.total, 1);

  // The subject-wise history entry is corrected in place, not appended.
  const agg = await StudentAttendance.findOne({ studentId: aliceRow!._id });
  const history = agg!.history as { date: string; status: string }[];
  assert.equal(history.length, 1);
  assert.equal(history[0].status, 'Absent');
});

test('attendance history for several enrollments loads in one request', async () => {
  const { cls, alice } = await seed();
  const teacher = await User.findOne({ role: 'teacher' });
  const other = await Class.create({ name: 'Embedded Systems', teacherId: teacher!._id });

  await post(h(enroll), '/api/enrollments', { userId: alice._id, classId: cls._id });
  await post(h(enroll), '/api/enrollments', { userId: alice._id, classId: other._id });

  for (const classId of [cls._id, other._id]) {
    await post(h(markAttendance), '/api/attendance', {
      classId,
      date: '2026-03-02',
      attendanceData: [{ studentId: alice._id, status: 'Present' }],
  ...
  const enrollments = await Student.find({ userId: alice._id });
  const ids = enrollments.map((e) => String(e._id));

  const res = await get(h(getAttendance), `/api/attendance?studentId=${ids.join(',')}`);
  assert.equal(res.status, 200);
  assert.equal(res.body.records.length, 2, 'both courses returned from a single call');
});

test('a malformed id inside a studentId list is rejected', async () => {
  await seed();
  const res = await get(h(getAttendance), '/api/attendance?studentId=abc,def');
  assert.equal(res.status, 400);
});
