...
import { badRequest, conflict, emailFilter, escapeRegex, ok, readJson, route } from '@/lib/http';
...
export const POST = route('Add faculty', async (request: NextRequest) => {
  ...
  if (await Faculty.exists({ email: emailFilter(email) })) {
    throw conflict('Faculty with this email already exists');
  }

  let user = await User.findOne({ email: emailFilter(email) });
  if (!user) {
