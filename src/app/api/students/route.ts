...
import { badRequest, conflict, notFound, objectId, ok, readJson, route } from '@/lib/http';
...
export const POST = route('Add student', async (request: NextRequest) => {
  ...
  const linkedUser = email ? await User.findOne({ email, role: 'student' }).select('_id') : null;
