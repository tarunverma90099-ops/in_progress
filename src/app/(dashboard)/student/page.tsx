...
function StudentDashboardContent() {
  ...
  const fetchData = useCallback(async (userId: string) => {
    try {
      ...
      // 2. Attendance history for every enrollment in ONE request (previously
      //    this fired a separate request per enrolled course).
      const subjectByEnrollment = new Map(
        studentEnrollments
          .filter((e) => e.class)
          .map((e) => [e._id, e.class!.name] as const)
      );

      const history: Record<string, Record<string, string>> = {};
      if (subjectByEnrollment.size > 0) {
        try {
          const result = await attendanceAPI.get({
            studentId: [...subjectByEnrollment.keys()],
          });
          for (const record of (result.records ?? []) as {
            ...
            const subject = subjectByEnrollment.get(String(record.studentId));
            if (!subject) continue;
            history[record.date] = { ...history[record.date], [subject]: record.status };
          }
        } catch (err) {
